        ...
        <div class="container mx-auto px-4">
            <div class="max-w-md mx-auto">
                ...
                <div class="text-center mb-6">
                    <div class="w-24 h-24 {{ $config['bgColor'] }} rounded-full flex items-center justify-center mx-auto mb-4">
                        <svg class="w-12 h-12 {{ $config['iconColor'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            {!! $config['icon'] !!}
                        </svg>
                    </div>
                    <h1 class="text-2xl font-bold text-neutral-900 mb-2">{{ $config['title'] }}</h1>
                    <p class="text-sm text-neutral-600">{{ $config['subtitle'] }}</p>
                    @if(isset($config['estimate']))
                        <p class="text-sm font-semibold text-secondary-600 mt-1">{{ $config['estimate'] }}</p>
                    @endif
                </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            <div class="flex flex-col items-center">
                                <div class="w-8 h-8 {{ $config['currentStep'] >= 1 ? 'bg-success-500' : 'bg-neutral-200' }} rounded-full flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="w-0.5 h-8 {{ $config['currentStep'] >= 2 ? 'bg-success-500' : 'bg-neutral-200' }} mt-1"></div>
                            </div>
                            <div class="flex-1 pt-1">
                                <p class="font-semibold {{ $config['currentStep'] >= 1 ? 'text-neutral-900' : 'text-neutral-500' }}">Pesanan Dikonfirmasi</p>
                                @if($config['currentStep'] >= 1)
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            <div class="flex flex-col items-center">
                                <div class="w-8 h-8 {{ $config['currentStep'] > 2 ? 'bg-success-500' : ($config['currentStep'] == 2 ? 'bg-primary-600' : 'bg-neutral-200') }} rounded-full flex items-center justify-center flex-shrink-0">
                                    @if($config['currentStep'] > 2)
                                        <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    @else
                                        <span class="{{ $config['currentStep'] == 2 ? 'text-white' : 'text-neutral-500' }} font-bold text-sm">2</span>
                                    @endif
                                </div>
                                <div class="w-0.5 h-8 {{ $config['currentStep'] >= 3 ? 'bg-success-500' : 'bg-neutral-200' }} mt-1"></div>
                            </div>
                            <div class="flex-1 pt-1">
                                <p class="font-semibold {{ $config['currentStep'] == 2 ? 'text-primary-700' : ($config['currentStep'] > 2 ? 'text-neutral-900' : 'text-neutral-500') }}">Teknisi Dalam Perjalanan</p>
                                @if($config['currentStep'] == 2)
                                    <p class="text-xs text-neutral-500 mt-0.5">Teknisi sedang menuju lokasi Anda</p>
                                @elseif($config['currentStep'] > 2)
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            <div class="flex flex-col items-center">
                                <div class="w-8 h-8 {{ $config['currentStep'] >= 3 ? 'bg-success-500' : 'bg-neutral-200' }} rounded-full flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1 pt-1">
                                <p class="font-semibold {{ $config['currentStep'] >= 3 ? 'text-neutral-900' : 'text-neutral-500' }}">Proses Perbaikan</p>
                                @if($config['currentStep'] >= 3)
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            <div class="flex flex-col items-center">
                                <div class="w-8 h-8 {{ $config['currentStep'] > 3 ? ($config['currentStep'] >= 4 ? 'bg-success-500' : 'bg-secondary-600') : 'bg-neutral-200' }} rounded-full flex items-center justify-center flex-shrink-0">
                                    @if($config['currentStep'] > 4)
                                        <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    @elseif($config['currentStep'] == 4)
                            ...
                            <div class="flex-1 pt-1">
                                <p class="font-semibold {{ $config['currentStep'] == 3 ? 'text-secondary-700' : ($config['currentStep'] > 3 ? 'text-neutral-900' : 'text-neutral-500') }}">Menunggu Pembayaran</p>
                                @if($config['currentStep'] == 3)
                                    <p class="text-xs text-neutral-500 mt-0.5">Silakan lakukan pembayaran</p>
                                @elseif($config['currentStep'] > 3)
                        ...
                        <div class="flex items-start gap-3">
                            <div class="flex flex-col items-center">
                                <div class="w-8 h-8 {{ $config['currentStep'] >= 4 ? 'bg-success-500' : 'bg-neutral-200' }} rounded-full flex items-center justify-center flex-shrink-0">
                                    @if($config['currentStep'] >= 4)
                                        <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                        </svg>
                            ...
                            <div class="flex-1 pt-1">
                                <p class="font-semibold {{ $config['currentStep'] >= 4 ? 'text-success-700' : 'text-neutral-500' }}">Selesai</p>
                                @if($config['currentStep'] >= 4)
                ...
                @if(isset($config['showPayment']) && $config['showPayment'])
                <div class="card p-4 mb-4">
                ...
                <div class="mt-4 space-y-3">
                    ...
                    <a href="{{ route('user.search') }}" class="btn btn-outline w-full">Kembali ke Beranda</a>
                </div>
                @endif
        ...
        function confirmCashPayment() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setTimeout(() => {
                        Swal.close();
                        window.location.href = '{{ route('user.order-tracking', ['orderId' => $orderId ?? 1]) }}?status=completed';
                    }, 1500);
                }
            });
        }
