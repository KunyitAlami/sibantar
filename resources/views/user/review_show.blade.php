<x-layout-user>
    <!-- Header -->
    <section class="bg-white border-b border-neutral-200 sticky top-0 z-50">
        <div class="container mx-auto px-4">
            <div class="flex items-center gap-4 py-4">
                <a href="{{ route('user.history', ['id_user' => Auth::id()]) }}" class="text-neutral-700 hover:text-primary-700">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </a>
                <h1 class="font-bold text-lg text-neutral-900">Lihat Review</h1>
            </div>
        </div>
    </section>

    <section class="container mx-auto px-4 py-6">
        <div class="max-w-xl mx-auto">
            <div class="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                <h2 class="text-lg font-bold text-neutral-900 mb-2">Review Anda</h2>

                <div class="text-sm text-neutral-600 mb-3">Bengkel: <span class="text-neutral-800 font-semibold">{{ $order->bengkel->nama_bengkel }}</span></div>
                <div class="text-sm text-neutral-500 mb-4">Tanggal Order: {{ $order->created_at }}</div>

                <div class="mb-3">
                    <div class="text-neutral-800 font-medium mb-1">Rating Bengkel</div>
                    <div class="flex items-center gap-1 text-2xl">
                        @for ($i = 1; $i <= 5; $i++)
                            @if ($i <= $review->ratingBengkel)
                                <span class="text-yellow-500">★</span>
                            @else
                                <span class="text-gray-300">★</span>
                            @endif
                        @endfor
                    </div>
                </div>

                <div class="mb-3">
                    <div class="text-neutral-800 font-medium mb-1">Rating Layanan</div>
                    <div class="flex items-center gap-1 text-2xl">
                        @for ($i = 1; $i <= 5; $i++)
                            @if ($i <= $review->ratingLayanan)
                                <span class="text-yellow-500">★</span>
                            @else
                                <span class="text-gray-300">★</span>
                            @endif
                        @endfor
                    </div>
                </div>

                <div class="mb-4">
                    <div class="text-neutral-800 font-medium mb-1">Komentar</div>
                    <div class="text-neutral-700 text-sm">{{ $review->comment ?? '-' }}</div>
                </div>

                <div class="mt-3">
                    <a href="{{ route('user.history', ['id_user' => Auth::id()]) }}" class="btn btn-outline w-full rounded-full">Kembali</a>
                </div>
            </div>
        </div>
    </section>

</x-layout-user>
